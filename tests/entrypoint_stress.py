                ...
                with open(log_file, "r") as f:
                    ...
                    while True:
                        ...
                        if "error" in line.lower() or "exception" in line.lower():
                            print(
                                f"\033[91m{line.strip()}\033[0m", flush=True
                            )  # Red for errors
                        elif "warn" in line.lower():
                            print(
                                f"\033[93m{line.strip()}\033[0m", flush=True
                            )  # Yellow for warnings
                        else:
...
def main():
    ...
    try:
        ...
        for i in range(1, num_iterations + 1):
            ...
            for test_id in test_ids:
                print(
                    f"Running test_{test_id}.run() function (iteration {i}/{num_iterations})...",
                    flush=True,
                )
                test_result = run_command(
                    f"dfx canister call test run_test {test_id}", timeout=180
                )

                if not test_result:
                    print(
                        f"Error: test_{test_id}.run() function timed out or failed",
                        flush=True,
                    )
                    # Print recent log entries to help with debugging
                    if os.path.exists(DFX_LOG_FILE):
                        ...
                        run_command(
                            f"tail -n 50 {DFX_LOG_FILE} | grep -i -E 'error|exception|fail'"
                        )
                    ...
                    print(
                        f"Error: test_{test_id}.run() function returned unexpected result: {test_result}",
                        flush=True,
                    )
                    # Print recent log entries to help with debugging
                    if os.path.exists(DFX_LOG_FILE):
                    ...
                    print(
                        f"Iteration {i} completed. Total entities inserted: {i * 100}",
                        flush=True,
                    )
